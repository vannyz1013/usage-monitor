...
const { cached } = require('./token-cache');

const AGENTS = ['claude', 'codex'];

// { today: {claude, codex}, month: {claude, codex}, days: {claude: {period: weighted}, codex} };
// month is null for an agent with no pay day.
// fresh: skip the 5-minute cache (lib/token-cache.js) - what the ⟳ button and Refresh now do.
function readTokens(now = new Date(), fresh = false) {
  ...
  // The cache key: the day, and the pay days that decide which days are counted.
  const key = `${ymd(now, '-')}|${ymd(since)}|${AGENTS.map((a) => (billing[a] || {}).payDay).join(',')}`;
  const report = cached(key, fresh, () => {
    try {
      return JSON.parse(execSync(`ccusage daily --json --offline --since ${ymd(since)}`, {
