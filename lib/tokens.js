...
const { ymd, weighted, billingCycle, readPayDays } = require('./month');
...
function readTokens(now = new Date()) {
  ...
  for (const a of AGENTS) {
    if (!payDays[a]) continue;
    cycles[a] = billingCycle(payDays[a], now);
  ...
  const month = {};
  for (const a of AGENTS) {
    if (cycles[a]) month[a] = { ...empty(), weighted: 0, start: ymd(cycles[a].start, '-'), end: ymd(cycles[a].end, '-') };
  }
